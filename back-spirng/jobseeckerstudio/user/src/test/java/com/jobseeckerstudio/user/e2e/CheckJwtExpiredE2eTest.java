...
public class CheckJwtExpiredE2eTest {
    ...
    private ResponseEntity<String>getResponse(HttpHeaders headers) {
        return rt.exchange("/api/v1/check-expired-jwt", HttpMethod.GET, new HttpEntity<>(headers), String.class);
    ...
    public void checkJwtExpiredWhenExpiredAccessToken() {
        ...
        HttpHeaders headers = createHeaders(expiredAccessToken.getJwtToken(), expiredAccessToken.getRefreshToken());
        ResponseEntity<String> response = getResponse(headers);
        DocumentContext dc =  JsonPath.parse(response.getBody());
        System.out.println(response.getBody());
        Integer status = dc.read("$.status");
    ...
    public void checkJwtExpiredWhenExpiredRefreshToken() {
        ...
        userRepository.save(user);
        HttpHeaders headers = createHeaders(expiredRefreshToken.getJwtToken(), expiredRefreshToken.getRefreshToken());
        ResponseEntity<String> response = getResponse(headers);
        DocumentContext dc =  JsonPath.parse(response.getBody());
        System.out.println(response.getBody());
        Integer status = dc.read("$.status");
        ...
        assertThat(userRepository.findByUserKey(user.getUserKey()).get().getSalt()).isEqualTo(responseToken);
    }
}
