...
//   ];
  ...
  async function createQuizList(page) {
    ...
    const url = new URL(`${quizSpringHost}/api/v1/category/${localStorage.getItem("categoryId")}/quiz`);
    ...
    quizData.forEach((quiz) => {
      quizListElement.innerHTML += `
        <li>
          <h5 class="mb-2 quiz-title" onclick="toggleQuizQuestions(${quiz.quizId})">${quiz.quizTitle}</h5>
          <ul class="choice-list" id="choice${quiz.quizId}" style="display:none;">
            ${quiz.choices
              ...
              .join("")}
            <button class="btn btn-outline-dark" id="edit-quiz-btn${quiz.quizId}">편집</button>
          </ul>
        </li>`;
    });
    ...
    for (let i = 0; i < quizData.length; i++) {
      const editBtn = document.getElementById("edit-quiz-btn" + quizData[i].quizId);
      if (editBtn) {
  ...
  function showModal(quiz) {
              ...
              <form id="editQuizForm">
                <div id="editQuestionsContainer">
                  <div class="mb-3">
                    ...
                    <input type="text" class="form-control" id="editTitle" placeholder="퀴즈 제목" value="${quiz.quizTitle}">
                  </div>
                  <div class="mb-3">
                      ...
                      .map(
                        (choice, index) =>
                          `<div class="form-check">
                             <input class="form-check-input" type="radio" name="correctAnswer" id="editOption${choice.choiceId}" value="${choice.choiceId}" ${choice.answer ? 'checked' : ''}>
                             <input type="text" id="updateQuizChoice${choice.choiceId}"class="form-control" value="${choice.choiceContent}">
                           </div>`
                      )
                      .join("")}
                  </div>
    ...
    const updatedChoices = Array.from(quiz.choices).map((choice, index) => {
      return {
        ...
        content: modalDiv.querySelector(`#updateQuizChoice${choice.choiceId}`).value,
...
function updateQuiz(updatedQuizData) {
  ...
  async function saveQuizzes() {
    ...
    for (let index = 0; index < containerList.length; index++) {
      const quizTitle = containerList[index].querySelector("#quizTitle").value;
      const quizChoice = [
      ...
      const checkAnswer = containerList[index].querySelector("#correctAnswer").value;
      const isAnswer = [
        checkAnswer == 1,
        checkAnswer == 2,
        checkAnswer == 3,
