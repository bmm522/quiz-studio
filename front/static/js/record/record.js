...
async function getRecordsAction(page) {
    ...
    if (categorySelect !== "all")
        url.searchParams.set("category", categorySelect);
    ...
    await fetch(url, { headers })
        ...
        .then((data) => {
            ...
            data.data._quizRecords.forEach((problem, index) => {
                ...
                const id = index;
                html += `
                  <tr class= "title-tr" data-status="${
                      quizIsAnswer !== false ? "resolved" : "unresolved"
                  }" onclick="toggleProblemDescription(${id})">
                      <td><div >${quizTitle}</div></td>
                      <td>${category}</td>
                      <td><span class="badge badge-${
                          quizIsAnswer !== false ? "success" : "warning"
                      }">${
                    quizIsAnswer !== false ? "해결됨" : "해결 못함"
                }</span></td>
                  </tr>
                      ...
                      <td colspan="3">
                          ...
                          <div class="form-check">
                            <label class="form-check-label ${
                                quizChoiceIsAnswer[index] === true
                                    ? "text-success"
                                    : ""
                            }" for="problem${id}${index}">
                              ${choice}
                            </label>
                          </div>
                        `,
...
function updatePagination(totalPages, currentPage) {
    ...
    for (let i = 1; i <= totalPages; i++) {
        paginationHtml += `<button class="btn btn${
            i === currentPage ? "-primary" : "-secondary"
        } mx-1" onclick="goToPage(${i})">${i}</button>`;
    }
...
function submitAnswer(id) {
    const radios = document.querySelectorAll(
        `#problemDescription${id} input[type="radio"]`,
    );
    let userChoice = null;
