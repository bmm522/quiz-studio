...
public class GetEmailE2ETest {
	...
	public void getEmailTestWhenUnregisteredUser() {
		User unregisteredUser = User.builder().userKey("rrrr")
			.email(Encryptor.encrypt("test_email@test.com")).build();
		JwtToken jwtTokenForUnregisteredUser = JwtMaker.create(unregisteredUser);
		HttpHeaders headers = createHeaders(jwtTokenForUnregisteredUser.getJwtToken(),
			jwtTokenForUnregisteredUser.getRefreshToken());
		DocumentContext dc = executeGetEmailRequest(headers);
